      ...
      <div className='grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3'>
        {filteredActivities.map((activity, i) => {
          ...
          return (
            ...
            >
              ...
              <div className='relative w-full p-3'>
                ...
                <div className={`tag absolute left-5 top-5 ${randomColor}`}>
                  {activity.type}
                </div>
              </div>
          ...
          className={`inline-block rounded-full px-3 py-1 text-xs font-medium mb-2
            ${activity.status === 'draft' ? 'bg-orange-100 text-orange-600' :
              activity.status === 'concluded' ? 'bg-gray-300 text-gray-800' : ''}
          `}
        >
          {activity?.status}
        </span>
                    ...
                    {formatToShortDate(activity?.startDateTime)} | {activity?.ambassadorName}
                    </p>
                    <p className='mb-2 text-sm'>
