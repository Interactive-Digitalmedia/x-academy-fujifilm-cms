...
export const EventTabs: React.FC<EventTabsProps> = ({
  ...
  useEffect(() => {
    if (activity) {
      setAmbassador(activity.ambassadorId || null)
  ...
  <>
    ...
    {activity?.about?.whatsIncluded?.length > 0 && (
      ...
      {activity.about.whatsIncluded.map((item: string, i: number) => {
        const cleanLabel = item.replace(/([A-Z])/g, ' $1').trim() // In case you still use camelCase
        const iconSrc = whatsIncludedIcons[cleanLabel] || null

        return (
          <li key={i} className="flex items-center gap-3 font-medium text-[15px]">
            {iconSrc && (
              <img
                src={iconSrc}
                alt={cleanLabel}
